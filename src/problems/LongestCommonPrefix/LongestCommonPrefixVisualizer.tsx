...
interface LongestCommonPrefixInput {
  strs: string[];
}

function LongestCommonPrefixVisualizer() {
  const visualization = useVisualization<LongestCommonPrefixInput>(
    (input) => generateLongestCommonPrefixSteps(input.strs),
    { strs: ["flower", "flow", "flight"] }
  );

  const variables = visualization.currentStepData?.variables;
  const charIndex = getNumberVariable(variables, 'charIndex');
  const checkingString = getNumberVariable(variables, 'checkingString');
  const matched = getBooleanVariable(variables, 'matched');
  const mismatch = getBooleanVariable(variables, 'mismatch');
  ...
  return (
    <VisualizationLayout
      ...
      inputFields={[{ type: "array", key: "strs", label: "字符串数组 strs (用逗号分隔)", placeholder: "输入字符串，用逗号分隔，如: flower,flow,flight" }]}
      testCases={[
    ...
    >
        ...
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
          ...
          <div className="space-y-3">
            {visualization.input.strs.map((str: string, strIndex: number) => {
