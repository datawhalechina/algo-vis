...
export interface VectorAddInput {
    ...
    dataSize: number;
    threadsPerBlock: number; // New parameter
    blockDim?: number; 
    gridDim?: number;  
}
...
export function generateVectorAddSteps(input: VectorAddInput): VisualizationStep[] {
    ...
    const dataC = new Array(dataSize).fill(null);
    ...
    steps.push({
        ...
        data: {
            host: { A: dataA, B: dataB, C: new Array(dataSize).fill(null) },
            device: { A: new Array(dataSize).fill(0), B: new Array(dataSize).fill(0), C: new Array(dataSize).fill(0) }, // 0 indicates allocated but empty/garbage
            activeThreads: [],
